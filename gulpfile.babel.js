...
gulp.task('default',
  gulp.series(watchTask, server, watch));

// Build emails, then send to litmus
gulp.task('litmus',
  gulp.series(watchTask, creds, aws, litmus));

// Build emails, then send to litmus
gulp.task('mail',
  gulp.series(watchTask, creds, aws, mail));

// Build emails, then zip
gulp.task('zip',
  gulp.series(watchTask, zip));
...
function sass() {
  ...
  return gulp.src('src/'+prefix+'assets/scss/app.scss')
...
function images() {
  ...
  return gulp.src('src/'+prefix+'assets/img/**/*')
    .pipe($.imagemin())
...
function watch() {
  ...
  gulp.watch(['../scss/**/*.scss', 'src/'+prefix+'assets/scss/**/*.scss']).on('change', gulp.series(resetPages, sass, pages, inline, browser.reload));
  gulp.watch('src/'+prefix+'assets/img/**/*').on('change', gulp.series(images, browser.reload));
}
